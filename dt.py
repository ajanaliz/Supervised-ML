...
def entropy(y):
    ...
    N = len(y)
    s1 = (y == 1).sum()
	"""shortcut if the number of 1s is 0 or N, then the 
	entropy is going to be 0"""
    if 0 == s1 or N == s1:
        return 0
    p1 = float(s1) / N
    p0 = 1 - p1
...
class TreeNode:
    def __init__(self, depth=0"""current depth""", max_depth=None):
        # print 'depth:', depth
        self.depth = depth
    ...
    def fit(self, X, Y):
        if len(Y) == 1 or len(set(Y)) == 1: # base case.
            ...
            best_col = None
            best_split = None
            for col in cols:# look for best column.
                ig, split = self.find_split(X, Y, col) # find best split for column.
                # print "ig:", ig
                if ig > max_ig:
                    max_ig = ig
                    best_col = col
            ...
            if max_ig == 0: # base case.
                ...
                if self.depth == self.max_depth: # base case.
                    ...
                    left_idx = (X[:,best_col] < best_split) # all the points for which X at this attribute is less than the split.
    ...
    def find_split(self, X, Y, col):
        ...
        x_values = X[:, col]
        sort_idx = np.argsort(x_values) # indexes which sort the x values.
    ...
    def predict_one(self, x):
        ...
        if self.col is not None and self.split is not None: # we actually made a split.
            feature = x[self.col]
